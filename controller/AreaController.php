<?php
	...
	class AreaController {
		...
		public function listarPorCodigo($codigo) {
			try {
				$consulta = "SELECT * FROM Area WHERE codigo_area = ".$codigo." ORDER BY codigo_area ASC";
				//ejecutando la consulta
				if($this->databaseTransaction != null) {
					$resultado = $this->databaseTransaction->ejecutar($consulta);
					if($this->databaseTransaction->cantidadResultados() == 0) {
					...
					if(ambiente == 'DEV') { echo "Controller Area - listarPorCodigo: El objeto DatabaseTransaction se encuentra nulo"; }
					return false;
				}
			}catch(Exception $e) {
				if(ambiente == 'DEV') { echo $e->getMessage(); }
		...
		public function listarPorNombre($nombre) {
			try {
				$consulta = "SELECT * FROM Area WHERE trim(lower(nombre_area)) = '".$nombre."' ORDER BY codigo_area ASC";
				//ejecutando la consulta
				if($this->databaseTransaction != null) {
					$resultado = $this->databaseTransaction->ejecutar($consulta);
					if($this->databaseTransaction->cantidadResultados() == 0) {
					...
					if(ambiente == 'DEV') { echo "Controller Area - listarPorNombre: El objeto DatabaseTransaction se encuentra nulo"; }
					return false;
				}
			}catch(Exception $e) {
				if(ambiente == 'DEV') { echo $e->getMessage(); }
		...
		public function ingresar($param) {
			try {
				...
				if($obj != null) {
					//construyendo string
					$consulta = "INSERT INTO Area ";
					$consulta = $consulta."(nombre_area, estado, fecha_creacion) VALUES ";
					$consulta = $consulta."('".$obj->getnombre_area()."', ".$obj->getestado().", '".date("Y-m-d H:i:s")."' );";
					//ejecutando la consulta
					if($this->databaseTransaction != null) {
						$resultado = $this->databaseTransaction->ejecutar($consulta);
						if($resultado == true) {
		...
		public function actualizar($param) {
			try {
				...
				if($obj != null) {
					...
					$consulta = $consulta."SET nombre_area = '".$obj->getnombre_area()."', estado = ".$obj->getestado()." ";
					$consulta = $consulta."WHERE codigo_area = ".$obj->getcodigo_area().";";
					//ejecutando la consulta
					if($this->databaseTransaction != null) {
						$resultado = $this->databaseTransaction->ejecutar($consulta);
						if($resultado == true) {
