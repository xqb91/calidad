<?php
	class LogEvaluacionParcialController {
		...
		public function listar() {
			try {
				$consulta = 'SELECT * FROM log_evaluacion_parcial';
		...
		public function listarPorCodigo($codigo) {
			try {
				$consulta = "SELECT * FROM log_evaluacion_parcial WHERE codigo = '".$codigo."'";
				//ejecutando la consulta
				if($this->databaseTransaction != null) {
					$resultado = $this->databaseTransaction->ejecutar($consulta);
					if($this->databaseTransaction->cantidadResultados() == 1) {
						return new LogEvaluacionParcial($this->databaseTransaction->resultados());
					}else{
						if(ambiente == 'DEV') { echo "LogEvaluacionParcialController - listarPorCodigo: La consulta SQL devolvió mas de un resultado por eso se devuelve NULL"; }
						return null;
					}
				}else{
