		...
		public function listar() {
			try {
				...
				if($this->databaseTransaction != null) {
					...
					if(ambiente == 'DEV') { echo "Controller Ciclo - Listar: El objeto DatabaseTransaction se encuentra nulo"; }
					return false;
				}
			}catch(Exception $e) {
				if(ambiente == 'DEV') { echo $e->getMessage(); }
			...
			try {
				...
				if($this->databaseTransaction != null) {
					...
					if($this->databaseTransaction->cantidadResultados() == 1) {
						return new Ciclo($this->databaseTransaction->resultados());
					}else{
						if(ambiente == 'DEV') { echo "Controller Ciclo - ListarPorCodigo: La consulta SQL devolvió mas de un resultado por eso se devuelve NULL"; }
						return null;
					}
				}else{
					if(ambiente == 'DEV') { echo "Controller Ciclo - ListarPorCodigo: El objeto DatabaseTransaction se encuentra nulo"; }
					return false;
				}
			}catch(Exception $e) {
				if(ambiente == 'DEV') { echo $e->getMessage(); }
			...
			try {
				...
				if($this->databaseTransaction != null) {
					...
					if($this->databaseTransaction->cantidadResultados() == 1) {
						...
						if(ambiente == 'DEV') { echo "Controller Ciclo - ListarPorSigla: La consulta SQL devolvió mas de un resultado por eso se devuelve NULL"; }
						return null;
					}
				}else{
					if(ambiente == 'DEV') { echo "Controller Ciclo - ListarPorSigla: El objeto DatabaseTransaction se encuentra nulo"; }
					return false;
				}
			}catch(Exception $e) {
				if(ambiente == 'DEV') { echo $e->getMessage(); }
