<?php
	class LogDetallelEvaluacionParcialController {
		...
		public function listar() {
			try {
				$consulta = 'SELECT * FROM log_detalle_evaluacion_parcial';
		...
		public function listarPorCodigo($codigo) {
			try {
				$consulta = "SELECT * FROM log_detalle_evaluacion_parcial WHERE codigo = '".$codigo."'";
				//ejecutando la consulta
				if($this->databaseTransaction != null) {
					$resultado = $this->databaseTransaction->ejecutar($consulta);
					if($this->databaseTransaction->cantidadResultados() == 1) {
						return new LogDetalleEvaluacionParcial($this->databaseTransaction->resultados());
					}else{
						if(ambiente == 'DEV') { echo "LogDetallelEvaluacionParcialController - listarPorCodigo: La consulta SQL devolvió mas de un resultado por eso se devuelve NULL"; }
						return null;
					}
				}else{
