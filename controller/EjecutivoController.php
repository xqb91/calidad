<?php
	...
	include(dirModel."Ejecutivo.php");
	class EjecutivoController {
		...
		public function listarPorEstado($estado) {
			try {
				$consulta = "SELECT * FROM ejecutivo WHERE codigo_estado = '".$estado."' ORDER BY nombre_ejecutivo ASC";
				//ejecutando la consulta
				if($this->databaseTransaction != null) {
					$resultado = $this->databaseTransaction->ejecutar($consulta);
					if($this->databaseTransaction->cantidadResultados() == 0) {
		...
		public function listarPorCorreo($correo) {
			try {
				$consulta = "SELECT * FROM ejecutivo WHERE correo = '".$correo."' ORDER BY nombre_ejecutivo ASC";
				//ejecutando la consulta
				if($this->databaseTransaction != null) {
					$resultado = $this->databaseTransaction->ejecutar($consulta);
					if($this->databaseTransaction->cantidadResultados() == 0) {
		...
		public function ingresar($param) {
			try {
				...
				if($obj != null) {
					//construyendo string
					$consulta = "INSERT INTO Ejecutivo ";
					$consulta = $consulta."(rut_ejecutivo, nombre_ejecutivo, fecha_inicio, codigo_estado, correo, codigo_ciclo, codigo_area, codigo_jornada) VALUES ";
					$consulta = $consulta."(".$obj->getrut_ejecutivo().", '".$obj->getnombre_ejecutivo()."', '".date('Y-m-d H:i:s')."', ".$obj->getcodigo_estado().",'".$obj->getcorreo()."', ".$obj->getcodigo_ciclo().", ".$obj->getcodigo_area().", ".$obj->getcodigo_jornada()." );";
					//ejecutando la consulta
					if($this->databaseTransaction != null) {
						$resultado = $this->databaseTransaction->ejecutar($consulta);
						if($resultado == true) {
		...
		public function actualizar($param) {
			try {
				...
				if($obj != null) {
					//construyendo string
					$consulta = "UPDATE Ejecutivo ";
					$consulta = $consulta."SET nombre_ejecutivo = '".$obj->getnombre_ejecutivo()."', fecha_inicio = '".$obj->getfecha_inicio()."', codigo_estado = ".$obj->getcodigo_estado().", correo = '".$obj->getcorreo()."', codigo_ciclo = ".$obj->getcodigo_ciclo().", codigo_area = ".$obj->getcodigo_area().", codigo_jornada = ".$obj->getcodigo_jornada()." ";
					$consulta = $consulta."WHERE rut_ejecutivo = ".$obj->getrut_ejecutivo().";";
					//ejecutando la consulta
					if($this->databaseTransaction != null) {
						$resultado = $this->databaseTransaction->ejecutar($consulta);
						if($resultado == true) {
		...
		public function eliminar($param) {
			try {
				...
				if($obj != null) {
					//construyendo string
					$consulta = "DELETE FROM Ejecutivo ";
					$consulta = $consulta."WHERE rut_ejecutivo = ".$obj->getrut_ejecutivo().";";
					//ejecutando la consulta
					if($this->databaseTransaction != null) {
						$resultado = $this->databaseTransaction->ejecutar($consulta);
