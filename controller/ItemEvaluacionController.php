<?php
	...
	include(dirModel."ItemEvaluacion.php");
	class ItemEvaluacionController {
		...
		public function listarPorCategoria($categoria) {
			try {
				$consulta = "SELECT * FROM item_evaluacion WHERE codigo_categoria = ".$categoria." ORDER BY codigo_categoria, orden ASC";
				//ejecutando la consulta
				if($this->databaseTransaction != null) {
					$resultado = $this->databaseTransaction->ejecutar($consulta);
					if($this->databaseTransaction->cantidadResultados() == 0) {
		...
		public function ingresar($param) {
			try {
				...
				if($obj != null) {
					//construyendo string
					$consulta = "INSERT INTO item_evaluacion ";
					$consulta = $consulta."(codigo_categoria, nombre_item, orden) VALUES ";
					$consulta = $consulta."(".$obj->getcodigo_categoria().", '".$obj->getnombre_item()."', ".$obj->getorden()." );";
					//ejecutando la consulta
					if($this->databaseTransaction != null) {
						$resultado = $this->databaseTransaction->ejecutar($consulta);
						if($resultado == true) {
						...
						if(ambiente == 'DEV') { echo "ItemEvaluacionController - ingresar: El objeto DatabaseTransaction se encuentra nulo"; }
						return false;
					}
				}else{
					if(ambiente == 'DEV') { echo "ItemEvaluacionController - ingresar: El objeto Adjunto (Model) se encuentra nulo"; }
					return false;
				}
			}catch(Exception $e) {
				if(ambiente == 'DEV') { echo $e->getMessage(); }
		...
		public function actualizar($param) {
			try {
				...
				if($obj != null) {
					//construyendo string
					$consulta = "UPDATE item_evaluacion ";
					$consulta = $consulta."SET codigo_categoria = ".$obj->getcodigo_categoria().", nombre_item = '".$obj->getnombre_item()."', orden = ".$obj->getorder()." ";
					$consulta = $consulta."WHERE codigo_item = ".$obj->getcodigo_item().";";
					//ejecutando la consulta
					if($this->databaseTransaction != null) {
						$resultado = $this->databaseTransaction->ejecutar($consulta);
						if($resultado == true) {
						...
						if(ambiente == 'DEV') { echo "ItemEvaluacionController - actualizar: El objeto DatabaseTransaction se encuentra nulo"; }
						return false;
					}
				}else{
					if(ambiente == 'DEV') { echo "ItemEvaluacionController - actualizar: El objeto Adjunto (Model) se encuentra nulo"; }
					return false;
				}
			}catch(Exception $e) {
				if(ambiente == 'DEV') { echo $e->getMessage(); }
		...
		public function eliminar($param) {
			try {
				...
				if($obj != null) {
					//construyendo string
					$consulta = "DELETE FROM item_evaluacion ";
					$consulta = $consulta."WHERE codigo_item = ".$obj->getcodigo_item().";";
					//ejecutando la consulta
					if($this->databaseTransaction != null) {
						$resultado = $this->databaseTransaction->ejecutar($consulta);
						...
						if(ambiente == 'DEV') { echo "ItemEvaluacionController - eliminar: El objeto DatabaseTransaction se encuentra nulo"; }
						return false;
					}
				}else{
					if(ambiente == 'DEV') { echo "ItemEvaluacionController - eliminar: El objeto Adjunto (Model) se encuentra nulo"; }
					return false;
				}
			}catch(Exception $e) {
				if(ambiente == 'DEV') { echo $e->getMessage(); }
