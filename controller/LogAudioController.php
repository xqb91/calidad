<?php
	class LogAudioController {
		...
		public function listar() {
			try {
				$consulta = 'SELECT * FROM log_audio';
		...
		public function listarPorCodigo($codigo) {
			try {
				$consulta = "SELECT * FROM log_audio WHERE codigo = '".$codigo."'";
				//ejecutando la consulta
				if($this->databaseTransaction != null) {
					$resultado = $this->databaseTransaction->ejecutar($consulta);
					if($this->databaseTransaction->cantidadResultados() == 1) {
						return new LogAudio($this->databaseTransaction->resultados());
					}else{
						if(ambiente == 'DEV') { echo "LogAudioController - listarPorCodigo: La consulta SQL devolvió mas de un resultado por eso se devuelve NULL"; }
						return null;
					}
				}else{
