<?php
	class DetalleEvaluacionParcialController {
		...
		public function listar() {
			try {
				$consulta = 'SELECT * FROM detalle_evaluacion_parcial';
		...
		public function listarPorNumeroItem($numero, $codigo_item) {
			try {
				$consulta = "SELECT * FROM detalle_evaluacion_parcial WHERE numeroEvaluacion = ".$numero." AND codigo_item = ".$codigo_item."";
				//ejecutando la consulta
				if($this->databaseTransaction != null) {
					$resultado = $this->databaseTransaction->ejecutar($consulta);
					if($this->databaseTransaction->cantidadResultados() == 1) {
						return new DetalleEvaluacionParcial($this->databaseTransaction->resultados());
					}else{
						if(ambiente == 'DEV') { echo "DetalleEvaluacionParcialController - listarPorNumeroItem: La consulta SQL devolvió mas de un resultado por eso se devuelve NULL"; }
						return null;
					}
				}else{
