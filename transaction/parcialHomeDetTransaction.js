...
$("#btnGenerarEva").click(function() {
	var ejecutivo = $("#btnGenerarEva").attr('ejecutivo');
	$.ajax({
		type: 'get', 
		url: 'core/SessionManager.php',
		error: function(XMLHttpRequest, textStatus, errorThrown) {
		...
		statusCode : {
			...
			200: function(responseObject, textStatus, errorThrown) {
					...
					$.ajax({
				        ...
				        data: 'periodo='+$("#slcPeriodo :selected").text()+'&ejecutivo='+irqljob,
				        beforeSend: function() {
				            //inicializando modal que valida sesión de raulí
				        },
						error: function(XMLHttpRequest, textStatus, errorThrown) {
				        ...
				        statusCode: {
				            	...
				            	$.ajax({
									...
									statusCode : {
										202: function(responseObject, textStatus, errorThrown) {
											$("#modalHomeConfig").attr('class', 'modal-dialog modal-xl');
						                	$("#modalHomeTitle").html('<i class="far fa-edit"></i> Crear Evaluación Parcial</strong>');
											$("#modalHomeContenido").load('creator.php?periodo='+$("#slcPeriodo :selected").text()+'&ejecutivo='+irqljob);
							            	$("#modalHomeBtnCerrar").show();
										        ...
										        statusCode: {
										            ...
										            200: function(responseObject, textStatus, errorThrown) {
										                ...
										                if(resultados.rut_evaluador == bckEvaluador.rut_evaluador) {
											                ...
											                $("#modalHomeContenido").attr('align', 'left');
											                $("#modalHomeCerrarVentana").show();
											                $("#modalHomeContenido").html('<p style="color:red;">Estimad@ '+resultados.nombre_evaluador+':<br />El ejecutivo al que piensas evaluar lo comenzó a evaluar <strong>'+bckEvaluador.nombre_evaluador+'</strong>. Si te has equivocado de ejecutivo, cancela esta operación de lo contrario haz click en proceder con nueva evaluación.</p>');
											                $("#modalHomeBtnCerrar").show();
											                $("#modalHomeBtnCerrar").text('Cancelar');
				            ...
				            200: function(responseObject, textStatus, errorThrown) {
				            	$("#modalHomeContenido").load('todoListoParcial.php?periodo='+$("#slcPeriodo :selected").text()+'&ejecutivo='+irqljob);
				            	$("#modalHomeBtnCerrar").show();
								$("#modalHomeBtnCerrar").text('Cerrar');
								$("#modalHomeCerrarVentana").show();
								$("#modalHomeBtnAccion").hide();
...
// ---- ZONA QUINCENAL ---------------------------------------------------------------------------------------------
			...
			statusCode : {
				...
				200: function(responseObject, textStatus, errorThrown) {
					$.ajax({
						...
						statusCode: {
							305: function(responseObject, textStatus, errorThrown) {
								$("#modalHomeTitle").text('Atención');
								$("#modalHomeContenido").html('La evaluación se ha guardado de forma correcta <strong>PERO LA EVALUACIÓN QUINCENAL FUE ELIMINADA</strong> ya que el sistema detectó que una o más evaluaciones que conformaban la evaluación quincenal fue(ron) eliminada(s) por lo tanto deberá regenerar la evaluación quincenal de forma manual!');
								$("#modalHomeBtnAccion").html('Regenerar Evaluación Quincenal Ahora');
								$("#modalHomeBtnAccion").show();
							},
							302: function(responseObject, textStatus, errorThrown) {
								$("#modalHomeTitle").text('Error al guardar la observación');
	...
	}else if($("#modalHomeBtnAccion").text() == "Regenerar Evaluación Quincenal Ahora") {
		$("#btnRegenerarQuincenal").click();
